		...
		if ("Notification" in window) {
			Notification.requestPermission().then((permission) => {
				...
				if (permission === "granted") {
					...
					toast.info("لطفاً نوتیفیکیشن‌ها را در تنظیمات مرورگر فعال کنید.");
				}
			});
		} else {
			toast.info("این مرورگر از نوتیفیکیشن پشتیبانی نمی‌کند.");
		}
	};
			...
			try {
				...
				await axiosService({
					...
					body: {
						type: "webpush",
						deviceInfo: {
							deviceId: navigator.userAgent, // یا یک کلید یکتا مثل fingerprintjs
				...
				toast.error("خطا در ثبت نوتیفیکیشن: " + error.message);
			}
		} else {
